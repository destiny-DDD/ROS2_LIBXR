...
RMSerialDriver::RMSerialDriver(const rclcpp::NodeOptions &options)
  ...
  sentry_ref_pub_ =
      this->create_publisher<referee_interfaces::msg::RobotStatus>(
          "referee/robot_status", rclcpp::QoS(rclcpp::KeepLast(1)));

  game_status_pub_ =
      this->create_publisher<referee_interfaces::msg::GameStatus>(
          "referee/game_status", rclcpp::QoS(rclcpp::KeepLast(1)));

  rfid_status_pub_ =
      this->create_publisher<referee_interfaces::msg::RfidStatus>(
          "referee/rfid_status", rclcpp::QoS(rclcpp::KeepLast(1)));

  sentry_state_pub_ =
      this->create_publisher<referee_interfaces::msg::SentryState>(
  ...
  set_pose_sub_ = this->create_subscription<referee_interfaces::msg::SetPose>(
      ...
      [this](const referee_interfaces::msg::SetPose::SharedPtr msg) {
        ...
        RCLCPP_INFO(this->get_logger(),
                    "SetPose received: pose=%d, forwarding to lower machine",
                    pose);
  ...
  //       "No game_progress==4 received in 5min, publishing fallback
  //       game_status=4");
  ...
  void (*ahrs_euler_cb_fun)(bool, RMSerialDriver *self, LibXR::RawData &data) =
      [](bool, RMSerialDriver *self, LibXR::RawData &data) {
        ...
        // std::cout<<"Serial got quat:"<<quat->w()<<","<<quat->x()<<","<<
        // quat->y()<<","<< quat->z()<<std::endl;

        rm_serial_driver::gimbal_euler gimbal_;
        self->convert_quaternion_to_euler(quat->x(), quat->y(), quat->z(),
                                          quat->w(), gimbal_.roll,
                                          gimbal_.pitch, gimbal_.yaw);
  ...
  void (*yawmotor_angle_cb_fun)(bool, RMSerialDriver *self,
                                LibXR::RawData &data) =
      [](bool, RMSerialDriver *self, LibXR::RawData &data) {
        auto quat = reinterpret_cast<float *>(data.addr_);
        self->yawmotor_angle_data = static_cast<float>(*quat);
      };
  auto yawmotor_angle_cb =
      LibXR::Topic::Callback::Create(yawmotor_angle_cb_fun, this);
  yawmotor_angle_topic_.RegisterCallback(yawmotor_angle_cb);

  /*哨兵裁判数据回调函数*/
  void (*sentry_ref_cb_fun)(bool, RMSerialDriver *self,
                            LibXR::RawData &data) = [](bool,
                                                       RMSerialDriver *self,
                                                       LibXR::RawData &data) {
    ...
    rs_msg.shooter_barrel_cooling_value =
        sentry_data->robot_status.shooter_barrel_cooling_value;
    rs_msg.shooter_barrel_heat_limit =
        sentry_data->robot_status.shooter_barrel_heat_limit;
    ...
    rs_msg.power_chassis_output =
        sentry_data->robot_status.power_chassis_output;
    rs_msg.power_launcher_output =
        sentry_data->robot_status.power_launcher_output;
    ...
    rfid_msg.friendly_trapezoidal_highland_gain_point =
        (rfid_bits & (1 << 3)) != 0;
    rfid_msg.enemy_trapezoidal_highland_gain_point =
        (rfid_bits & (1 << 4)) != 0;
    ...
    rfid_msg.friendly_central_highland_lower_gain_point =
        (rfid_bits & (1 << 9)) != 0;
    rfid_msg.friendly_central_highland_upper_gain_point =
        (rfid_bits & (1 << 10)) != 0;
    rfid_msg.enemy_central_highland_lower_gain_point =
        (rfid_bits & (1 << 11)) != 0;
    rfid_msg.enemy_central_highland_upper_gain_point =
        (rfid_bits & (1 << 12)) != 0;
...
/*四元数转欧拉角*/
void RMSerialDriver::convert_quaternion_to_euler(float qx, float qy, float qz,
                                                 float qw, float &roll,
...
/*底盘运动数据回调函数*/
void RMSerialDriver::get_classic(
    const geometry_msgs::msg::Twist::SharedPtr twi) {
